  ...
  .map(function(student) {
    return new Promise(function(resolve, reject) {
      upsertUser(student, function(err, userInfo, user) {
        ...
        debug('finding teachers ' + student.coaches)
        User.findAll({ where: { khanLongId: { $in: student.coaches } } })
        .then(function(teachers) {
          if (!teachers) return
          debug('adding ' + teachers.length + ' teachers to ' + user.id)
          return user.addTeachers(teachers)
        }).then(function() {
          resolve(user)
        })
      })
